...
export const generateQuestions = async () => {
  ...
  try {
    ...
    const response = await generateObject({
      ...
      system:
        "You are a helpful assistant that generates interview questions for a given technology and difficulty level. Each question should have a 'text' field and a 'choices' array of 4 objects, each with 'text' and 'isCorrect' (boolean, only one isCorrect should be true per question).",
